...
%     title(' envolvente interpolada.')
    ...
    env_refe_int_d_aux = filter(fir_dem2,1,env_refe_int_d_aux);
...
%             plot(env_med)
    ...
    %         title("Env Int medida")
            ...
            %suavizado de le derivada
            env_med_int_d = filter(fir_dem2,1,env_med_int_d);

            %normalizacion de la derivada
            [max_d_m max_i_m] = max(env_med_int_d );
            env_med_int_d = (1/max_d_m).*env_med_int_d;
...
%             title("Derivada de la medicion")
    ...
    %         title("tramo 2 med")
            ...
            %[TOF1 TOF2]
               ...
               % Distancia(count_dist,count_med) =340000*[(TOF2-TOF1)*1/(3*fs)]
...
%save('D:\Google Drive\TESIS FM\Scrips Matlab\psoc_med_dis\int_prom_tia_gauss.mat','TOF1_v','TOF2_v','Distancia','temperatura')
save('D:\Google Drive\TESIS FM\Scrips Matlab\psoc_med_dis\int_prom_dem2_tia_gauss.mat','TOF1_v','TOF2_v','Distancia','temperatura')

%% Graficcar los resultados 

% ref = (1:1:4);
% 
% aux = load('D:\Google Drive\TESIS FM\Psoc\Datos_mediciones\med_distancia.mat')
% figure
% plot(ref,aux.distancia)
% 
% figure
% error = ref - aux.distancia;
% plot(ref, error, '*')
