%% medidor de distancia "NORMAL", el algoritmo inicial 
...
%[B,A] = butter(5,[0.15 0.25],'stop');
%[B1,A1] = butter(5,[0.35 0.45],'stop');
%[B,A] = butter(7,0.1,'low');
...
env_refe = filter(B,A,env_r);
%env_refe = filter(B2,A2,env_refe);
...
env_refe_d = filter(numd,dend,env_refe);%se deriva la envolvelnte interpolada, derivada de amstrong

%se suaviza la derivada con un filtro fir paso bajos 
...
% Esta ya es la version 2 de OnA
...
while(count_dist < 22)
    ...
    %         title(msg,'FontSize',15,'Color',[1 1 0]);
           ...
           % set(figureHandle,'Visible','on');
            ...
            env_med = filter(B,A,env_m);
            %env_med = filter(B2,A2,env_med);
    %         figure
    %         plot(eco_med)
    ...
    %         plot(env_med)
            ...
            %se suaviza la derivada con un filtro fir paso bajos 
            env_med_d = filter(fir_dem2,1,env_med_d);
...
save('D:\Google Drive\TESIS FM\Scrips Matlab\psoc_med_dis\normal_dem2_tia_gauss.mat','TOF1_v','TOF2_v','Distancia','temperatura')
